...
def generate_move(num_moves=10, depth=4):
    """
    Generate chess moves using the loaded ChessNet model, opening book, minimax, or random selection.
    ...
    Returns:
        ...
        str: The source of the generated move (model, book, minimax, or random).
    ...
    try:
        ...
        # Get top moves from the model output
        top_moves = torch.topk(output.flatten(), num_moves).indices
        
        # Find the first legal move among the top moves
        for move_label in top_moves:
            move = label_to_move(move_label.item())
            if move in board.legal_moves:
                move_san = board.san(move)
                logger.info(f"AI move applied: {move_san}")
                return [move_san], "model"
        
        # If no legal moves found from the model, use minimax
        logger.info("No legal moves from model, using minimax")
        maximizing_player = (board.turn == ai_color)
        _, best_move = minimax(board, depth, float('-inf'), float('inf'), maximizing_player)
        
        if best_move:
            logger.info(f"Move generated by minimax: {best_move.uci()}")
            return [board.san(best_move)], "minimax"
        
        # If minimax fails, fall back to random move
